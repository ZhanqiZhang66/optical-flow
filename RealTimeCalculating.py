...
iCountMovement, iCountMovement_plusSTD, iCountMovement_minusSTD = np.zeros(length), np.zeros(length), np.zeros(length)
frames = []
all_angles, magnitudes = [], []
iThreshold = 0
fieldnames = ["frames", "number of Movements", "number of Movements Error"]
...
while (cap.isOpened()):
    with open(output_data_name, 'a') as csv_file:
        ...
        #iMouse_mag = magnitude[99:1001, 567:1473]  # the blue area
        ...
        iCountMovement[count] = len([i for i in flattened if i > iThreshold + 2* iThresholdstd])
        ...
        info = {
            "frames": count,
            "number of Movements": iCountMovement[count],
            "number of Movements Error": abs(iCountMovement_plusSTD[count]  - iCountMovement_minusSTD[count] )
        }
...
plt.hist(background_area, color = 'blue', edgecolor = 'black',
         bins = int(180/5))

# Add labels
plt.title('Histogram of Background Flow')
